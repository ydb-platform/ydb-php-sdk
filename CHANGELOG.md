* fixed `Serialization of 'Closure' is not allowed` when a logger is set - `Iam::token_temp_file()` serializes the whole config to build a cache filename, and `Ydb::__construct()` attaches the logger onto the credentials object, so any real logger holding a `Closure` internally (common - processors, handlers, formatters) broke every request. `Auth::__sleep()` now drops any object-valued property before serializing, which also covers `StaticAuthentication`'s own nested `Ydb` instance. Also fixed the test's fixture logger using a typed property and an arrow function, both PHP 7.4+ only, a syntax error on this SDK's own stated PHP 7.2 minimum - found via this fork's own GitHub Actions run against the real CI matrix.
